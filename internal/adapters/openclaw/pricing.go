...
// silently wrong $0. OpenClaw records real token counts but always a $0 cost
// (subscription/CLI plan, not per-token billing), so this recomputes an
// estimated-equivalent cost from the tokens — a comparison proxy, not money
// charged (see internal/pricing doc).
